...
from olympe.messages.ardrone3.PilotingState import moveToChanged, FlyingStateChanged, PositionChanged, AttitudeChanged, moveByChanged
...
from olympe.messages.rth import set_min_altitude, set_ending_hovering_altitude, set_ending_behavior, return_to_home, state, rth_auto_trigger
...
#	
#	#gps
	goto_gps(drone, max_height, min_height, take_off_altitute)
...
def goto_gps(drone, max_height, min_height, take_off_altitute):
...
#	list_of_points = [[47.62185,-122.17840,50.000000], [47.62192,-122.17847,50.000000]]
	...
	for point in list_of_points:
		print(point)
		drone(
		    moveTo(point[0],  point[1], point[2] - take_off_altitute + max_height, MoveTo_Orientation_mode.TO_TARGET, 0.0)
...
#		    moveTo(point[0],  point[1], point[2] - take_off_altitute + min_height, MoveTo_Orientation_mode.TO_TARGET, 0.0)
		    extended_move_to(point[0],  point[1], point[2] - take_off_altitute + min_height, MoveTo_Orientation_mode.TO_TARGET, 0.0, 4.0, 4.0, 30.0)
		    >> moveTo(point[0],  point[1], point[2] - take_off_altitute + min_height, MoveTo_Orientation_mode.HEADING_START, 0.0)
		    >> moveToChanged(status='DONE', _timeout=10, _policy="check_wait")
		    ).wait().success()
		for i in range(4):   
			print(i)     
